...
export class EmailNotificationHandler {
  ...
  > = async (payload) => {
    try {
      ...
      if (!clientProfile?.user?.email) {
        console.warn(`Session reminder: Client ${payload.clientId} has no email`);
        return;
      }
      ...
      console.log(`Session reminder email sent to ${clientProfile.user.email}`);
    } catch (error) {
  ...
  > = async (payload) => {
    try {
      ...
      if (!clientProfile?.user?.email) {
        console.warn(`Resource shared: Client ${payload.clientId} has no email`);
        return;
      }
      ...
      console.log(`Resource shared email sent to ${clientProfile.user.email}`);
    } catch (error) {
